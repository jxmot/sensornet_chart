    ...
    $.Zebra_DatePicker = function(element, options) {
        ...
        var defaults = {
            ...
            init = function(update) {
                ...
                if (!(plugin.settings.always_visible instanceof jQuery)) {
                    ...
                    if (!update) {
                        ...
                        if (plugin.settings.show_icon) {
                            ...
                            icon = $('<button onmousedown="event.preventDefault()" type="button" class="Zebra_DatePicker_Icon' + ($element.attr('disabled') === 'disabled' ? ' Zebra_DatePicker_Icon_Disabled' : '') + '">Pick a date</button>');
                ...
                if(plugin.settings._view_days_only === true) {
                    $('.dp_caption', header).addClass('dp_viewoff');
                }
                ...
                datepicker
                    .on('mouseover', 'td:not(.dp_disabled,.dp_actionoff,.dp_viewoff)', function() {
                        $(this).addClass('dp_hover');
                    })
                    .on('mouseout', 'td:not(.dp_disabled,.dp_actionoff,.dp_viewoff)', function() {
                ...
                $(plugin.settings.rtl ? '.dp_next' : '.dp_previous', header).on('click', function() {
                    ...
                    if($('.dp_previous', header).hasClass('dp_actionoff')) return;
                ...
                if (plugin.settings.fast_navigation)
                    ...
                    $('.dp_caption', header).on('click', function() {
                        ...
                        if((view === 'days') && (plugin.settings._view_days_only === true))
                            return;
                ...
                $(plugin.settings.rtl ? '.dp_previous' : '.dp_next', header).on('click', function() {
                    ...
                    if($('.dp_next', header).hasClass('dp_actionoff')) return;
            ...
            generate_daypicker = function() {
                ...
                $('.dp_next', header).removeClass('dp_actionoff');
                $('.dp_previous', header).removeClass('dp_actionoff');
                ...
                for (i = 0; i < 42; i++) {
                    ...
                    if ((plugin.settings.rtl && day < 1) || 
                       (!plugin.settings.rtl && i < days_from_previous_month)) {

                        html += '<td class="dp_not_in_month ' + 
                                (is_weekend ? 'dp_weekend ' : '') + 
                                (plugin.settings.select_other_months && !is_disabled(real_year, real_month, real_day) ? 'dp_available date_' + real_date : 'dp_disabled') + 
                                '">' + 
                                (plugin.settings.select_other_months || plugin.settings.show_other_months ? str_pad(rtl_offset + days_in_previous_month - days_from_previous_month + i + 1, plugin.settings.zero_pad ? 2 : 0) : '&nbsp;') + 
                                '</td>';
                    ...
                    // if this is a day from the next month
                        ...
                        html += '<td class="dp_not_in_month ' + 
                                ...
                                (plugin.settings.select_other_months && !is_disabled(real_year, real_month, real_day) ? 'dp_available date_' + real_date : 'dp_disabled') + 
                                '">' + 
                                (plugin.settings.select_other_months || plugin.settings.show_other_months ? str_pad(day - days_in_month, plugin.settings.zero_pad ? 2 : 0) : '&nbsp;') + 
                                '</td>';
                    ...
                    // if this is a day from the current month
                        ...
                        if (selected_month === current_system_month && selected_year === current_system_year && current_system_day === day) {
                            ...
                            if(plugin.settings._limit_month_selection === true) {
                                $('.dp_next', header).removeClass('dp_hover');
                                $('.dp_next', header).addClass('dp_actionoff');
                            }
                        }
                        ...
                        } else { // jxmot - allows for extra formatting, like color and bold
                            if(!class_name.includes('dp_current') && !class_name.includes('dp_selected'))
                                class_name += ' dp_enabled';
                        }
                    ...
                    if(plugin.settings._limit_month_selection === true) {
                        if(day < 0) {
                            var tmp = plugin.settings.direction[0];
                            var lowday = replaceAll(tmp, '-', '');
                            if(lowday > real_date) {
                                $('.dp_previous', header).removeClass('dp_hover');
                                $('.dp_previous', header).addClass('dp_actionoff');
                            }
                        }
                    }
                }
             ...
             *  @return string
             *
             *  @param  string      src     The string to search within
             *  @param  string      find    The substring to find
             *  @param  string      repl    The replacement for what we find
             *
             *  @access private
             */
            replaceAll = function(src, find, repl) {
                var escapedFind=find.replace(/([.*+?^=!:${}()|\[\]\/\\])/g, "\\$1");
                return src.replace(new RegExp(escapedFind, 'g'), repl);
            },
